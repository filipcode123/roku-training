sub init()
    ...
    m.helpRectangle = m.top.findNode("helpRectangle")
    ...
    m.closeButton.observeField("buttonSelected","closeHelpRectangle")
    m.sign_in_buttons.setFocus(True)
    ' m.top.findNode("longText").setFocus(True)
end sub

sub closeHelpRectangle()
    m.helpRectangle.visible = "false"
    m.row_list.visible = "true"
    m.button_group_1.setFocus(True)
end sub

sub revealHelpRectangle()
    m.helpRectangle.visible = "true"
...
function onKeyEvent(key as String, press as Boolean) as Boolean  
    ...
    if press = true then
        if key = "right" and m.button_group_1.isInFocusChain() then
            if m.button_group_1.buttonFocused = 3 then
                revealHelpRectangle()
            else
